...
public class NotificationPluginCordovaImpl extends BaseNotificationPlugin implements  NotificationPlugin {
	...
	private native void registerDeviceImpl(JavaScriptObject pushNotification, boolean alertParamter, boolean badgeParamter,
		...
		pushNotification.registerDevice({
			...
			that.@nz.govt.studylink.mslapp.client.cordova.plugins.NotificationPluginCordovaImpl::handleOnRegisterDeviceSuccess(Lcom/google/gwt/core/client/JavaScriptObject;)(status);
		}, function(status) {
			// Fail callback, get the error message
			var statusString = JSON.stringify(status);
			// Callback to java method
			that.@nz.govt.studylink.mslapp.client.cordova.plugins.NotificationPluginCordovaImpl::handleOnRegisterDeviceFail(Lcom/google/gwt/core/client/JavaScriptObject;)(status);
		});
	}-*/;
	...
	private native JavaScriptObject init()/*-{
		...
		instance.notificationCallback = function(notification){
			...
			that.@nz.govt.studylink.mslapp.client.cordova.plugins.NotificationPluginCordovaImpl::handleOnPushNotification(Lcom/google/gwt/core/client/JavaScriptObject;)(notification);
		};
