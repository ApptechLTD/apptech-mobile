...
// Update by pengfei
// transmit the pushstart flag to the javascript to indicate the app status
- (void) onPushAccepted:(PushNotificationManager *)manager withNotification:(NSDictionary *)pushNotification onStart:(BOOL)onStart{
    ...
    NSMutableDictionary* mslPushNotification = [[NSMutableDictionary alloc] initWithDictionary:pushNotification];
    NSString* onStartStr = [NSString stringWithFormat:@"%@", (onStart ? @"true" : @"false")];
    [mslPushNotification setValue: onStartStr forKey:@"pushStart"];
    
	PW_SBJsonWriter * json = [[PW_SBJsonWriter alloc] init];
	NSString *jsonString =[json stringWithObject:mslPushNotification];
	//[json release]; json = nil;
	//[mslPushNotification release];
    
	NSString *jsStatement = [NSString stringWithFormat:@"window.plugins.pushNotification.notificationCallback(%@);", jsonString];
    NSLog(@"PushNotification.onPushAccepted callbackjs = %@", jsStatement);
