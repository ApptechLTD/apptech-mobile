...
- (void) sendDevTokenToServer:(NSString *)deviceID {
    // Register the device on push woosh server, only trigger when we want to use PushWoosh server for testing
#ifdef PUSHWOOSH_TEST
...
- (BOOL) handlePushReceived:(NSDictionary *)userInfo {
    ...
    NSLog(@"PushNotificationManager.handlePushReceived userInfo = %@", userInfo);
    
	BOOL isPushOnStart = NO;
	NSDictionary *pushDict = [userInfo objectForKey:@"aps"];
	if(!pushDict) {
        NSLog(@"PushNotificationManager.handlePushReceived pushStart set to YES because pushDict");
	...
	if([[UIApplication sharedApplication] respondsToSelector:@selector(applicationState)] && [[UIApplication sharedApplication] applicationState] != UIApplicationStateActive) {
		isPushOnStart = YES;
        NSLog(@"PushNotificationManager.handlePushReceived pushStart set to YES because applicationState");
	}
...
- (BOOL)application:(UIApplication *)application newDidFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
	...
	if(result) {
		...
		if(userInfo) {
            ...
            // Add the pushstart flag into json
            NSMutableDictionary* mslPushUserInfo = [[NSMutableDictionary alloc] initWithDictionary:userInfo];
            [mslPushUserInfo setValue: @"true" forKey:@"pushStart"];
            
			NSString *jsonString =[json stringWithObject:mslPushUserInfo];
			//[json release]; json = nil;
          //  [mslPushUserInfo release];
